...
class CoresAdapter : RecyclerView.Adapter<CoresAdapter.ViewHolder>() {
    ...
    private var mCoresData: List<Core>? = null
    ...
    override fun getItemCount(): Int {
        return mCoresData?.size ?: 0
    }

    // Todo after item layout change, refactor finding views
    class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        val coreSerialTextView: TextView = itemView.findViewById(R.id.text_capsule_serial)
        val coreBlock: TextView = itemView.findViewById(R.id.text_capsule_type)
        val coreStatus: TextView = itemView.findViewById(R.id.text_capsule_status)
        val coreLandings: TextView = itemView.findViewById(R.id.text_landings)
        val coreMissionsTextView: TextView = itemView.findViewById(R.id.text_missions)
    ...
    fun setData(data: List<Core>?) {
        ...
        mCoresData = data?.toMutableList()?.asReversed()
